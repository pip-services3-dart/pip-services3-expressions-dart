import 'package:pip_services3_expressions/src/mustache/parsers/MustacheToken.dart';
import 'package:pip_services3_expressions/src/mustache/tokenizers/tokenizers.dart';
...
class MustacheParser {
    ...
    if (_originalTokens.isNotEmpty) {
      ...
      if (_hasMoreTokens()) {
        ...
        throw MustacheException(
            ...
            'Syntax error near ' + (token?.value).toString(),
            token?.line ?? 0,
            token?.column ?? 0);
      }
      _lookupVariables();
    ...
    while (_hasMoreTokens()) {
      ...
      if (token?.type == MustacheTokenType.SectionEnd) {
        throw MustacheException(
            ...
            "Unexpected section end for variable '" +
  ...
  List<MustacheToken> _performSyntaxAnalysisForSection(String? variable) {
    ...
    while (_hasMoreTokens()) {
      ...
      if (token?.type == MustacheTokenType.SectionEnd) {
        throw MustacheException(
            null,
            MustacheErrorCode.UnexpectedSectionEnd,
            "Unexpected section end for variable '" + variable.toString() + "'",
            token?.line ?? 0,
            token?.column ?? 0);
      }
    ...
    throw MustacheException(
        ...
        "Not closed section for variable '" + variable.toString() + "'",
        token?.line ?? 0,
        token?.column ?? 0);
  }
